...
RandomForestPipeline_New = R6Class(
  ...
  public = list(
    ...
    #       Observed = if ("Ton" %in% names(data_pred)) data_pred$Ton else NA,
    #       Predicted = self$predictions,
    #       #Labor_Nr_Bobo_ID = if ("Labor-Nr Bobo_ID" %in% names(data_pred)) data_pred$`Labor-Nr Bobo_ID` else NA
    #       Labor_Nr_Bobo_ID = Labor_Nr_Bobo_ID
    #       )))
    #   }
    #   
    ...
    #   
    #   
    #   # LOSO predictions tracking erstellen wenn LOSO split verwendet wird
    ...
    run_parallel_loso = function(sites = NULL, implementation = "schnell", mtry = NULL, 
                                 cv_method = "repeatedcv", cv_folds = 5, cv_repeats = 5,
                                 num_trees = 1000, importance_method = "permutation", 
                                 tune_depth = "standard", workers = NULL, split_method = "LOSO") {
      # Sites zu verarbeiten
      sites_to_process = if(is.null(sites)) self$site_list else sites
      
      self$log_info(sprintf("Starte parallele LOSO-Validierung für %d Sites", length(sites_to_process)))
      ...
      # Anzahl der Arbeits-Prozesse bestimmen
      if(is.null(workers)) {
        workers = parallel::detectCores() - 1
      } else {
        workers = min(workers, parallel::detectCores())
      }
      ...
      self$log_info(sprintf("Parallele Ausführung mit %d Worker-Prozessen", workers))
      
      # Variablen für Worker-Prozesse kopieren
      data_copy = self$data
      target_copy = self$target
      features_copy = self$features
      
      # WICHTIG: Exportiere alle notwendigen Funktionen und Variablen explizit
      # Das kann Probleme mit der Übertragung von R-Funktionen wie na.omit lösen
      if(requireNamespace("future.globals", quietly = TRUE)) {
        options(future.globals.maxSize = 1024*1024^2)  # 1 GB Limit
      } else {
        self$log_info("Paket 'future.globals' nicht installiert. Verwende Standardeinstellungen.")
      }  # Erhöhe das Limit für Datenübertragung
      
      # Erstelle einen sicheren Kontext für parallele Ausführung
      self$log_info("Erstelle Future-Kontext für parallele Verarbeitung...")
      
      # Parallel über alle Sites iterieren
      #---------#---------#---------#---------#---------#---------#---------#---------#
      site_results = future_lapply(sites_to_process, function(site) {
        # Pakete in Worker-Prozess laden
        ...
        tryCatch({
          # Erstelle eine neue Pipeline-Instanz
          pipeline = RandomForestPipeline_New$new(
            data = data_copy,
            target = target_copy,
            features = features_copy
          )
          
          # LOSO-Split durchführen
          pipeline$split(test_site = site, split_method = split_method)
          
          # Vereinfachtes Training ohne do.call
          if (implementation == "schnell") {
            # Manuelle Grid-Erstellung für Ranger
            n_features = length(features_copy)
            
            # MTry-Werte bestimmen
            if (is.null(mtry)) {
            ...
            } else if (is.character(mtry) && mtry == "effizient") {
              mtry_values = unique(round(c(
                sqrt(n_features),
                n_features/3,
                n_features/2
            ...
            ctrl = trainControl(
              method = cv_method,
              number = cv_folds,
              repeats = if(cv_method == "repeatedcv") cv_repeats else NULL,
              allowParallel = TRUE
            )
            
            # Formel erstellen
            formula = as.formula(paste(target_copy, "~", paste(features_copy, collapse = " + ")))
            
            # Direktes Aufrufen von train ohne do.call
            ...
            # Standard RF
            n_features = length(features_copy)
            mtry_grid = expand.grid(mtry = round(seq(1, min(n_features, 15), length.out = 5)))
          ...
          pipeline$predict()
          metrics = pipeline$evaluate(return_metric = TRUE)
          predictions = pipeline$get_loso_predictions()
          
          # Variable Importance extrahieren, wenn verfügbar
          var_imp = NULL
          best_tune = NULL
          
          if (!is.null(pipeline$model)) {
            best_tune = pipeline$model$bestTune
            tryCatch({
              var_imp = varImp(pipeline$model)
            }, error = function(e) {
          ...
          return(list(
            ...
            best_params = best_tune
          ))
        }, error = function(e) {
          # Bei Fehler, Fehlerinformation zurückgeben
          return(list(
            site = site,
            error = TRUE,
      ...
      # Parallel über alle Sites iteriere ende 
      #---------#---------#---------#---------#---------#---------#---------#---------#
      ...
      for (i in seq_along(site_results)) {
        result = site_results[[i]]
        site_name = result$site
        
        if (!is.null(result$error) && result$error) {
          self$log_info(sprintf("FEHLER bei Site %s: %s", site_name, result$error_message))
          failed_sites = c(failed_sites, site_name)
          next
        }
        
        all_metrics[[site_name]] = result$metrics
        
        # Nur wenn Vorhersagen vorhanden sind
        if (!is.null(result$predictions) && nrow(result$predictions) > 0) {
        ...
        # Variable Importance speichern, wenn verfügbar
        if (!is.null(result$var_importance)) {
          all_importance[[site_name]] = result$var_importance
        }
        
        # Beste Parameter speichern
        if (!is.null(result$best_params)) {
          all_best_params[[site_name]] = result$best_params
        }
        
        self$log_info(sprintf("Ergebnisse für Site %s:", site_name))
      ...
      if (length(all_metrics) > 0) {
        overall_metrics = data.frame(
          RMSE = mean(sapply(all_metrics, function(x) x$RMSE)),
          MAE = mean(sapply(all_metrics, function(x) x$MAE)),
          R2 = mean(sapply(all_metrics, function(x) x$R2)),
          RPD = mean(sapply(all_metrics, function(x) x$RPD)),
          Bias = mean(sapply(all_metrics, function(x) x$Bias))
        )
        
        # Standardabweichungen berechnen
        metrics_sd = data.frame(
          RMSE_sd = sd(sapply(all_metrics, function(x) x$RMSE)),
          MAE_sd = sd(sapply(all_metrics, function(x) x$MAE)),
          R2_sd = sd(sapply(all_metrics, function(x) x$R2)),
          RPD_sd = sd(sapply(all_metrics, function(x) x$RPD)),
          Bias_sd = sd(sapply(all_metrics, function(x) x$Bias))
        )
        
        self$log_info("Gesamtergebnisse:")
        print(overall_metrics)
        self$log_info("Standardabweichungen:")
        print(metrics_sd)
      } else {
        overall_metrics = NULL
        metrics_sd = NULL
        self$log_info("Keine Ergebnisse verfügbar!")
      }
      
      # Fehlgeschlagene Sites anzeigen
      if (length(failed_sites) > 0) {
        self$log_info("Fehlgeschlagene Sites:")
        print(failed_sites)
      }
      
      # Erweiterte Rückgabe mit Variablenwichtigkeit und besten Parametern
      return(list(
        site_results = all_metrics,
    ...
    # Methode um LOSO predictions zu erhalten
    get_loso_predictions = function() {
      if (is.null(self$loso_predictions)) {
        stop("Keine LOSO Vorhersagen vorhanden")
      }
      return(self$loso_predictions)
    },
